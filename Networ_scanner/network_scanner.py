...
def scan_single_host():
	...
	try:
		...
		print("\n________single_host_details__________")
		for port in scan["scan"][ip_address]['tcp'].items():
			print(f"{port[0]},state: {port[1]['state']},Reason :{port[1]['reason']},Product :{port[1]['product']},Version:{port[1]['version']} , Name: {port[1]['name']}")
	except:
...
def scan_range():
	...
	print("Wait.......................")
	try:
		scan = nm.scan(hosts=ip_address,arguments = "-sS -O -Pn") #Returns Dictionary
		#print(scan)
		print("\n__________________scan_range_______________")
		for port in scan["scan"][ip_address]['tcp'].items():
